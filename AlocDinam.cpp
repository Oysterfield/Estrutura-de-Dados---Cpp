...
Aluno *turmas_uniao(Aluno t1[], Aluno t2[], int N1, int N2) {
    ...
    for (int k = 0; k < N1 + N2; k++) {
        if (k < N1) {
        tu[j].escreveNome(t1[k].devolveNome());
        tu[j].escreveMatricula(t1[k].devolveMatricula());
        } else {
        tu[j].escreveNome(t2[k].devolveNome());
        tu[j].escreveMatricula(t2[k].devolveMatricula());
        }
        j++;
    }
    */
